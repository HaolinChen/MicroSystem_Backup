...
Matirx* M_mul2(Matirx* _mat_left,Matirx* _mat_right){/*Matrix Multiply
	矩阵点乘
	_mat_result = _mat_left.*_mat_right */
	/*Determine_Matrix_Dimensions*/
	Matirx* _mat_result = NULL;
...
Matirx* M_T(Matirx* _mat_source){/*Transpose
	转置*/
	Matirx* _mat = (Matirx*)malloc(sizeof(Matirx));//动态分配内存空间
...
Matirx* M_zero(Matirx* _mat){/*Matrix Zero mean
	对矩阵零均值化
	每一行都减去这一行的均值 */
	Matirx* _mat_result = NULL;
...
Matirx* M_multi_k(Matirx* _mat,MATRIX_TYPE k){/*Matrix Multiply by a constant k
	矩阵乘以常数 k  */
	Matirx* _mat_result = NULL;
	_mat_result = Matrix_copy(_mat);
	int row = _mat->row;

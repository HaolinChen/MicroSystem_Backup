...
{
    int i;
    double fft_output[1024],inputi[1024],fftr[1024],ffti[1024];
    //generate the input data
    Array_zero(1024,1,&fft_output[0]);
    for (i=0; i<=frame_size; i++)
    {
		fft_output[i]=signal->data[idx*frame_size+i];
	}
    kfft(fft_output,inputi,1024,10,fftr,ffti);  //FFT
    double final_output[frame_size];
    for (i=0; i<=frame_size; i++)
    {
		final_output[i]=fft_output[i];
	}
	Matrix* final_vector = Matrix_gen(1024,1,&final_output[0]);
	return final_vector;
}

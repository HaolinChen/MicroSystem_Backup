...
static PI_L2 uint8_t send_start = 0xfa;
static PI_L2 uint8_t send_stop = 0xfb;
uint8_t send_check;
...
void writeDetImg(unsigned char *imageinchar, uint16_t Img_num, int16_t score)
{
    ...
    sprintf(imgName, "../../../OUTPUT/num%d_score%f.ppm", Img_num, score_fp);
...
// Send person position without gesture to STM32 Add by Xinglong
            ...
            person_value = x + (y << 8) + (w << 16) + (h << 24);
            send_check = x + y + w + h;
            gesture_value = 0;
            // printf("No gesture : x %d y %d w %d h %d \n", x, y, w, h);
            // toggle LED when sending information
            PRINTF("\n-------- get done ----------\n");
            pi_gpio_pin_write(&gpio_device, 2, led_val);
            led_val ^= 1;
            pi_uart_write(&uart_device, &send_start, 1);
            pi_uart_write(&uart_device, &person_value, 4);
            pi_uart_write(&uart_device, &send_check, 1);
            pi_uart_write(&uart_device, &send_stop, 1);
            PRINTF("\n-------- uart done ----------\n");
            // pi_uart_write(&uart_device, "-----------no gesture-------------\n", 40);
            // sprintf(person_value_char, "%d\n", person_value);
            // sprintf(gesture_value_char, "%d\n", gesture_value);
#endif
        }
        else
        {
            ...
            printf("Has gesture : x %d y %d w %d h %d \n", x, y, w, h);
            pi_uart_write(&uart_device, &person_value, 4);
            pi_uart_write(&uart_device, &gesture_value, 1);
            // pi_uart_write(&uart_device, "-----------yes gesture-------------\n", 40);
            // sprintf(person_value_char, "%d\n", person_value);
            // sprintf(gesture_value_char, "%d\n", gesture_value);
            // pi_uart_write(&uart_device, &person_value_char, 20);
            // pi_uart_write(&uart_device, &gesture_value_char, 10);
#endif

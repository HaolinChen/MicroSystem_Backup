...

{
	...
	Ker Arg: In, Tiled Space: Tile0
		...
		KerArgItSpace: 4 logical tiles, 4 physical tiles
			Total Size: 11520 [Tile0, 4:[18x104, 2:18x104, 18x8], 2]
		KerArgItSpace (User Kernel Iter Order):
			[Tile0, 4:[18x104, 2:18x104, 18x8], 2]
		Tile0: [0, 3744, 208], Tile1: [208, 3744, 208], Tile2; [416, 3744, 208]
	...
	/*================================= Read Tiles Prolog ===============================*/
	AT_HYPERRAM_CL_COPY2D(&HyperRam, ((AT_HYPERRAM_EXT_ADDR_TYPE) In+0), ((AT_HYPERRAM_INT_ADDR_TYPE) SSDKernels_L2_Memory+0+0), 3744, 640, 208, 0, &UchanHR1);
	AT_HYPERRAM_CL_WAIT(&HyperRam, &UchanHR1); /* Wait previous uDMA read In */
	AT_HYPERRAM_CL_COPY2D(&HyperRam, ((AT_HYPERRAM_EXT_ADDR_TYPE) In+208), ((AT_HYPERRAM_INT_ADDR_TYPE) SSDKernels_L2_Memory+0+3744), 3744, 640, 208, 0, &UchanHR1);
	...
	for (T0Ind=0; T0Ind<4; T0Ind++, T0Ind_Total++) { /* Iteration on Tile0 */
		...
		if (!(T0Ind_Last)) {
			if (!(T0Ind_NextLast)) {
				_NN_In = _NN_In + (208); _LNN_In = ((T0Ind_NextNextLast)?16:208); _SNN_In = (18*_LNN_In); 
			}
		}
		/*============================= End Prepare Tiles ===================================*/
		/*================================= Read Tiles ======================================*/
		...
		KerArg0->Win = (unsigned int ) ((T0Ind_Last)?8:104);
		KerArg0->Out = (short * __restrict__) (SSDKernels_L1_Memory+7488+3744*((T0Ind_Total)%2));
		KerArg0->Hout = (unsigned int ) (T0Ind_Last?8:104);
		...
		_SC_Out = 0;
		if (!(T0Ind_Last)) {
			_C_Out = _C_Out + (3744); _SC_Out = ((T0Ind_NextLast)?288:3744); 
		}
		/*============================= End Prepare Tiles ===================================*/
...
{
	/* Shared L1: 14984 bytes, L2 buffer: 14984 bytes */
	/* Local variables used by this kernel */
	AT_L2_EVENT DmaR_Evt1;
	...
	Ker Arg: Classes, Tiled Space: Tile0
		Min Pipe Depth: 0, Max Pipe Depth: 2
		KerArgItSpace: 4 logical tiles, 4 physical tiles
			Total Size: 11520 [Tile0, 4:[3x535, 2:3x535, 3x315], 2]
		KerArgItSpace (User Kernel Iter Order):
			[Tile0, 4:[3x535, 2:3x535, 3x315], 2]
		Tile0: [0, 3210, 3210], Tile1: [3210, 3210, 3210], Tile2; [6420, 3210, 3210]
	Ker Arg: Boxes, Tiled Space: Tile0
		Min Pipe Depth: 0, Max Pipe Depth: 2
		KerArgItSpace: 4 logical tiles, 4 physical tiles
			Total Size: 15360 [Tile0, 4:[4x535, 2:4x535, 4x315], 2]
		KerArgItSpace (User Kernel Iter Order):
			[Tile0, 4:[4x535, 2:4x535, 4x315], 2]
		Tile0: [0, 4280, 4280], Tile1: [4280, 4280, 4280], Tile2; [8560, 4280, 4280]
	======================== End Ker Arg Iter Spaces =========================================*/
	/*=========================== Call Kernel, Invariant assignment =====================*/
	KerArg0->Classes_W = (unsigned int ) (3);
	KerArg0->Classes_Std_H = (unsigned int ) (535);
	...
	/*================================= Read Tiles Prolog ===============================*/
	AT_HYPERRAM_CL_COPY(&HyperRam, ((AT_HYPERRAM_EXT_ADDR_TYPE) Classes+0), ((AT_HYPERRAM_INT_ADDR_TYPE) SSDKernels_L2_Memory+0+0), 3210, 0, &UchanHR1);
	AT_HYPERRAM_CL_WAIT(&HyperRam, &UchanHR1); /* Wait previous uDMA read Classes */
	AT_HYPERRAM_CL_COPY(&HyperRam, ((AT_HYPERRAM_EXT_ADDR_TYPE) Classes+3210), ((AT_HYPERRAM_INT_ADDR_TYPE) SSDKernels_L2_Memory+0+3212), 3210, 0, &UchanHR1);
	AT_L2_COPY(0, ((AT_HYPERRAM_EXT_ADDR_TYPE) SSDKernels_L2_Memory+0+0), ((AT_L2_INT_ADDR_TYPE) SSDKernels_L1_Memory+0+0), 3210, 0, &DmaR_Evt1);
	_NN_Classes=3210; _SN_Classes=3210;
	AT_HYPERRAM_CL_COPY(&HyperRam, ((AT_HYPERRAM_EXT_ADDR_TYPE) Boxes+0), ((AT_HYPERRAM_INT_ADDR_TYPE) SSDKernels_L2_Memory+6424+0), 4280, 0, &UchanHR2);
	AT_HYPERRAM_CL_WAIT(&HyperRam, &UchanHR2); /* Wait previous uDMA read Boxes */
	AT_HYPERRAM_CL_COPY(&HyperRam, ((AT_HYPERRAM_EXT_ADDR_TYPE) Boxes+4280), ((AT_HYPERRAM_INT_ADDR_TYPE) SSDKernels_L2_Memory+6424+4280), 4280, 0, &UchanHR2);
	AT_L2_COPY(0, ((AT_HYPERRAM_EXT_ADDR_TYPE) SSDKernels_L2_Memory+6424+0), ((AT_L2_INT_ADDR_TYPE) SSDKernels_L1_Memory+6424+0), 4280, 0, &DmaR_Evt2);
	_NN_Boxes=4280; _SN_Boxes=4280;
	/*============================= End Read Tiles Prolog ===============================*/
	for (T0Ind=0; T0Ind<4; T0Ind++, T0Ind_Total++) { /* Iteration on Tile0 */
		int T0Ind_Last = (T0Ind==3), T0Ind_NextLast = ((T0Ind+1)==3), T0Ind_NextNextLast = ((T0Ind+2)==3);
		/*================================= Prepare Tiles ===================================*/
		...
		if (!(T0Ind_Last)) {
			if (!(T0Ind_NextLast)) {
				_NN_Classes = _NN_Classes + (3210); _SNN_Classes = ((T0Ind_NextNextLast)?1890:3210); 
			}
		}
		_SNN_Boxes = 0;
		if (!(T0Ind_Last)) {
			if (!(T0Ind_NextLast)) {
				_NN_Boxes = _NN_Boxes + (4280); _SNN_Boxes = ((T0Ind_NextNextLast)?2520:4280); 
			}
		}
		/*============================= End Prepare Tiles ===================================*/
		/*================================= Read Tiles ======================================*/
		AT_HYPERRAM_CL_WAIT(&HyperRam, &UchanHR1); /* Wait previous uDMA read Classes */
		if (_SNN_Classes) {
			AT_HYPERRAM_CL_COPY(&HyperRam, ((AT_HYPERRAM_EXT_ADDR_TYPE) Classes+_NN_Classes), ((AT_HYPERRAM_INT_ADDR_TYPE) SSDKernels_L2_Memory+0+3212*((T0Ind_Total)%2)),
					_SNN_Classes, 0, &UchanHR1);
		}
		AT_L2_WAIT(0, &DmaR_Evt1); /* Wait previous DMA read Classes */
		if (_SN_Classes) {
			AT_L2_COPY(0, ((AT_HYPERRAM_EXT_ADDR_TYPE) SSDKernels_L2_Memory+0+3212*((T0Ind_Total+1)%2)), ((AT_L2_INT_ADDR_TYPE) SSDKernels_L1_Memory+0+3212*((T0Ind_Total+1)%2)),
					_SN_Classes, 0, &DmaR_Evt1);
		}
		AT_HYPERRAM_CL_WAIT(&HyperRam, &UchanHR2); /* Wait previous uDMA read Boxes */
		if (_SNN_Boxes) {
			AT_HYPERRAM_CL_COPY(&HyperRam, ((AT_HYPERRAM_EXT_ADDR_TYPE) Boxes+_NN_Boxes), ((AT_HYPERRAM_INT_ADDR_TYPE) SSDKernels_L2_Memory+6424+4280*((T0Ind_Total)%2)),
					_SNN_Boxes, 0, &UchanHR2);
		}
		AT_L2_WAIT(0, &DmaR_Evt2); /* Wait previous DMA read Boxes */
		if (_SN_Boxes) {
			AT_L2_COPY(0, ((AT_HYPERRAM_EXT_ADDR_TYPE) SSDKernels_L2_Memory+6424+4280*((T0Ind_Total+1)%2)), ((AT_L2_INT_ADDR_TYPE) SSDKernels_L1_Memory+6424+4280*((T0Ind_Total+1)%2)),
					_SN_Boxes, 0, &DmaR_Evt2);
		}
		/*============================= End Read Tiles ======================================*/
		/*====================== Call Kernel LOC_LOOP =========================*/
		KerArg0->Classes = (short * __restrict__) (SSDKernels_L1_Memory+0+3212*((T0Ind_Total)%2));
		KerArg0->Boxes = (short * __restrict__) (SSDKernels_L1_Memory+6424+4280*((T0Ind_Total)%2));
		KerArg0->Classes_H = (unsigned int ) (T0Ind_Last?315:535);

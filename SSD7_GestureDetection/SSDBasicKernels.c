...
void KerSDD3Dto2DShort(KerSDD3Dto2DShort_ArgT *Arg)
{
    ...
    if (Q)
    {
        ...
        ChunkCell = ChunkSize((Hin / n_classes) * Win);
        First = CoreId * ChunkCell * n_classes, Last = Min(Win * Hin, First + ChunkCell * n_classes);
        if (First < Last)
        {
            for (unsigned int i = First; i < Last; i += n_classes)
            {
                ...
                // {
                //     printf("ChunkCell: %d \n", ChunkCell);
                //     printf("\n Core:%d Num:%d Confidence0: %f Confidence1: %f Confidence2: %f\n", CoreId, i, FIX2FP(Out[i],15), FIX2FP(Out[i+1],15), FIX2FP(Out[i+2],15));
                // }
            }
        }
...
void KerPredecoderShort(KerPredecoderShort_ArgT *Arg)
{
    ...
    for (unsigned int i = First; i < Last; i++)
    {
        ...
        if (Classes[i * n_classes + 7] > 9830) // person threshold = FP2FIX(0.4,15)
        {
            ...
            KerEstimate_bbox(i, TileIndex * Std_H + i, Classes[i * n_classes + 7], Boxes, Boxes_Q, anch, bbxs, 7);
        }

        for (unsigned int j = 1; j < 7; j++) //We have 6 gestures
        {
            if (Classes[i * n_classes + j] > 9830) // gesture threshold = FP2FIX(0.30,15)
            {
                if (bbxs->num_bb >= MAX_BB)
                {
                    printf("Reached Max BB number...\n");
                ...
                KerEstimate_bbox(i, TileIndex * Std_H + i, Classes[i * n_classes + j], Boxes, Boxes_Q, anch, bbxs, j);
            }
        }
    }
}
...
//             //we start from 1 since we skip the Background
//             //Here it would be nice to add a different confidence for each class
//             if (Classes[i * n_classes + 7] > 13107) // person threshold = FP2FIX(0.4,15)
//             {
//                 if (bbxs->num_bb >= MAX_BB)
//                 {
//                     printf("Reached Max BB number...\n");

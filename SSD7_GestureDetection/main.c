...
uint16_t max_person_confidence = 0;
short max_gesture_id = 0;
uint16_t max_gesture_confidence = 0;
uint8_t ssd_class = 0;
...
void writeDetImg(unsigned char *imageinchar, uint16_t Img_num, int16_t score, uint8_t label)
{

    static char imgName[120];
    //Save Images to Local
    float score_fp = FIX2FP(score, 15);
    sprintf(imgName, "../../../samples_codetest/gap8_gesture_prepare/pic_output/%d/num%d_score%f_label%d.pgm", gesture_now, Img_num, score_fp, label);
...
void writeDetImg_resnet(unsigned char *imageinchar, uint16_t Img_num, int outclass1)
{
    ...
    sprintf(imgName, "../../../OUTPUT/ssd_resnet_camera/optimization/gap8/num%ld_label%ld.pgm", Img_num, outclass1);
    PRINTF("Dumping image %s\n", imgName);
    ...
    for (idx = 0; idx < boundbxs->num_bb; idx++)
    {
        ...
        for (idx_int = 0; idx_int < boundbxs->num_bb; idx_int++)
        {
            if (boundbxs->bbs[idx_int].alive == 0 || idx_int == idx)
                continue;
            if (boundbxs->bbs[idx_int].class != boundbxs->bbs[idx].class && (boundbxs->bbs[idx_int].class == 7 || boundbxs->bbs[idx].class == 7))
                continue;
...
static void RunSSD()
{
    ...
    SDD3Dto2DSoftmax_20_16_48(Output_1, tmp_buffer_classes, OUTPUT_1_Q, 8);
    ...
    SDD3Dto2DSoftmax_10_8_48(Output_3, tmp_buffer_classes, OUTPUT_3_Q, 8);
    ...
    SDD3Dto2DSoftmax_5_4_48(Output_5, tmp_buffer_classes, OUTPUT_5_Q, 8);
    ...
    {
        if (bbxs.bbs[i].class == 7 && max_person_confidence < bbxs.bbs[i].score)
        {
            max_person_confidence = bbxs.bbs[i].score;
            max_person_id = i;
        }
        else if (bbxs.bbs[i].class < 7 && max_gesture_confidence < bbxs.bbs[i].score)
        {
    ...
    if (max_gesture_confidence != 0)
    {
        ...
        ssd_class = bbxs.bbs[max_gesture_id].class;
    }
...
int start()
{
    ...
    pi_ram_alloc(&HyperRam, &Output_1, 48 * 16 * 20 * sizeof(short int));
    pi_ram_alloc(&HyperRam, &Output_2, 24 * 16 * 20 * sizeof(short int));
    pi_ram_alloc(&HyperRam, &Output_3, 48 * 8 * 10 * sizeof(short int));
    pi_ram_alloc(&HyperRam, &Output_4, 24 * 8 * 10 * sizeof(short int));
    pi_ram_alloc(&HyperRam, &Output_5, 48 * 4 * 5 * sizeof(short int));
    pi_ram_alloc(&HyperRam, &Output_6, 24 * 4 * 5 * sizeof(short int));

    // Output_3 = (short int *) pmsis_l2_malloc( 12 * 30 * 30 * sizeof(short int));

    pi_ram_alloc(&HyperRam, &tmp_buffer_classes, 48 * 16 * 20 * sizeof(short int));
    ...
    while (iter)
    {
        ...
        if (SAVE_DET == 1)
        {
            ...
            writeDetImg(ImageInChar, count, max_person_confidence, ssd_class);
        }
        //-------------End Draw BBs-------------------//

...
void KerSDD3Dto2DShort(KerSDD3Dto2DShort_ArgT *Arg)
{
    ...
    if (Q)
    {
        ...
        for (unsigned int i = First; i < Last; i += n_classes)
            SoftMax_fp16(&Out[i], n_classes, &Out[i], Q);
...

{
    ...
    for (int i = 0; i < i_size; i++)
        Out[i] = Abs(gap_roundnorm_reg(Out[i] * InvSum, 15));
}
/*
Input information (For example: layer_4 postprocessing)
index: row of box_now in confidence matrix which is 2D:490x2 (3D:10x7x14) or box matrix which is 2D:490x4
global_index: global location of box_now
confidence: box_now's person confidence (We don't care the confidence of background)
Boxes: boxes matrix which is  2D:490x4 (3D:10x7x28)
Boxes_Q: quantization bit of layer_4 output
Anchors: points to anchor_layer_4 (for example) in SSDParams.c
bbxs: matirx to save the final results which is x_center, y_center, w, h
class: the class of box_now (0:background 1:person)
*/
void KerEstimate_bbox(int index, int global_index, int confidence, short int *Boxes, int Boxes_Q, Alps *Anchors, bboxs_t *bbxs, int class)
{
    ...
    anchor_location = global_index / Anchors->n_anchors; // row of box_now in 10x7 feature_map_4
    anchor_id = global_index % Anchors->n_anchors; // column of box_now in 10x7 feature_map_4
    ...
    cx_offset = Boxes[index * Anchors->anchor_params + 0]; // backbone output 0
    cy_offset = Boxes[index * Anchors->anchor_params + 1]; // backbone output 1
    w_offset = Boxes[index * Anchors->anchor_params + 2]; // backbone output 2
    h_offset = Boxes[index * Anchors->anchor_params + 3]; // backbone output 3
    //printf("%f\n",FIX2FP(w_offset,11));
    //printf("%f\n",FIX2FP(h_offset,11));
    ...
    //unsigned int Exp = Exp_fp_17_15((In[i]-M) << (15 - Norm));
    unsigned int Exp;
    Exp = Exp_fp_17_15((w_offset * var_2_f) >> (Boxes_Q - 5));  // do e^x calculation

...
void kf_test(void)
{
    ...
    time_end = rt_time_get_us(); // End timer
    time_total = time_end - time_start;
    printf("\nTotal time(uSec): %10d.", time_total);
    printf("\n-------------- initial mean --------------\n");
    mat_show(&mean[0], 1, 8);
    printf("\n-------------- initial covariance --------------\n");
...
// pi_display_write(&ili, &buffer, 40, 60, 160, 120);
        ...
        printf("FC(Mhz): %d, CL(Mhz): %d, total time(uSec): %10d.\n", FREQ_FC / 1000000, FREQ_CL / 1000000, total_time);

#if SOT_TEST
        kf_test();
#endif

        pi_gpio_pin_write(&gpio_device, 2, gpio_val);
        gpio_val ^= 1;
    }
